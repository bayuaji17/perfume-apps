...
import { useCreatePerfume } from "@/hooks/use-perfumes";
...
export default function AddPerfume() {
  ...
  const onSubmit = (data: z.infer<typeof formScheme>) => {
    ...
    const imageFilesOnly = images.filter(
  ...
  return (
    <div>
      <div className="flex w-full items-center gap-2 mb-2">
        <Button onClick={() => router.back()} className="w-20">
          <MoveLeft size={30} />
          <span>Back</span>
        </Button>
        <h1 className="before:content-['/'] text-2xl font-bold">
          Add New Perfume
        </h1>
      </div>
